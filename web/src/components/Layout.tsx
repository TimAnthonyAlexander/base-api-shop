...
import { useGetBasket, useGetMe } from '../hooks';
...
export default function Layout() {
  ...
  const { data: meData, error: meError } = useGetMe(undefined, { 
    enabled: true,
    onError: () => setIsAuthenticated(false),
  });

  const { data: basketData } = useGetBasket(undefined, { 
  ...
  const basketItemCount = (basketData?.data as any)?.items?.length || 0;
