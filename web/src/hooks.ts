 ...
 * Auto-fetches on mount and when dependencies change
 */
export function useGetMe(query?: Types.GetMeQueryParams, options?: QueryOptions<Types.GetMeResponse>, deps?: DependencyList): QueryResult<Types.GetMeResponse> {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const result = await Api.getMe(query, options);
      setData(result);
      options?.onSuccess?.(result);
    } catch (err) {
      const error = err instanceof Error ? err : new Error(String(err));
...
/**
 * React hook for POST /me
 * Returns a mutate function that must be called manually
 */
export function usePostMe(
  options?: QueryOptions<Types.PostMeResponse>
): MutationResult<Types.PostMeResponse, {body: Types.PostMeRequestBody}> {
  const [data, setData] = useState<Types.PostMeResponse | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);

  const mutate = useCallback(async (variables: {body: Types.PostMeRequestBody}) => {
    setLoading(true);
    setError(null);
    
    try {
      const result = await Api.postMe(variables.body, options);
      setData(result);
      options?.onSuccess?.(result);
      return result;
    } catch (err) {
